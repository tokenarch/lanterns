...
def resolve_gate_sequence(ctx: BootstrapContext, args: Dict[str, Any]) -> Tuple[str, str]:
    ...
    body = (
        "Gate sequence (run in order; all must pass before a change ships):\n"
        "\n"
        ...
        "     for every SCR rule (SCR-01..11 + CL5).\n"
        "  5. ``python3 scripts/nightclaw-ops.py validate-bundles`` — ``RESULT:PASS``\n"
        "     for every bundle declared in bundles.yaml.\n"
        "  6. ``python3 scripts/nightclaw-ops.py integrity-check`` — every\n"
        "     protected file matches its manifest hash (11/11 PASS).\n"
        "  7. ``python3 scripts/nightclaw-ops.py preflight-import`` —\n"
        "     Every Python module imports cleanly; STEP_CMD_MAP <-> ALLOWED_TIERS\n"
        "     consistency check passes. Catches the 'bridge broken for 3 days'\n"
        "     class of regression at the next session boundary.\n"
        "  8. ``python3 scripts/nightclaw-ops.py syntax-check`` —\n"
        "     ``ast.parse()``s every .py file under the four packages,\n"
        "     nightclaw_common, scripts/, and tests/. Catches NTFS write\n"
        "     truncation directly.\n"
        "\n"
        "Plus: ``python3 scripts/skills-sync.py`` must be a no-op after any edit\n"
        "under ``nightclaw_engine/`` or ``nightclaw_common/`` (Invariant 9 fails CI\n"
        "otherwise).\n"
        "\n"
        "Two operator commands available for incident response (not gates):\n"
        "  * ``crash-recover`` — evidence-based dead-holder lock recovery.\n"
        "    Wired into worker startup; safe to invoke manually if a lock is\n"
        "    stuck after a session crash.\n"
        "  * ``replay <RUN-YYYYMMDD-NNN> [--from Tx --to Tx --json]`` —\n"
        "    chronological reconstruction of one run from AUDIT-LOG +\n"
        "    CHANGE-LOG. Use this to debug a crashed pass instead of grepping\n"
        "    logs by hand."
    )
